...
export async function onStatusLatestOrderStatusDefaultGenerator(existingPayload: any, sessionData: any) {
  // Update context timestamp
  if (existingPayload.context) {
    existingPayload.context.timestamp = new Date().toISOString();
  }   
  
  console.log("existingPayload on status", existingPayload);
